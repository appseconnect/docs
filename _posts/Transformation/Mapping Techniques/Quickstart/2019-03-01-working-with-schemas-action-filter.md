...
**Note: The user must save each node individually. There is no way the user can save it on the whole.This process is same for both the XML and JSON formats.**
...
while the sync process is on and displays it in the agent.

For Query-Based Application (unlike SAP Business One) & SOAP Application, Resync Filters are needed should be defined in the portal itself. 
For REST Based Applications, the Resync filters are called up from the adapter itself. However, the portal still has the privilege of providing
Resync Filters even for a REST Application if required.
...
6.	Click `ADD ACTION` button for providing Actions in the schema level. 
