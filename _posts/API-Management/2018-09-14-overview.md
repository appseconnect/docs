...
- You need an Active [APPSeCONNECT Account](/appseconnect/home/). Go to Home Page for login details
...
1. Login to [APPSeCONNECT Portal](/appseconnect/home/). Go to Home Page for login details
2. Go to API management from the sidebar.
3. Select `Register URL`.
4. A popup window will open which will allow user to select a particular API `URL`(Uniform Resource Locator).

![Register Url Org](/staticfiles/api-management/media/register-url-org.PNG)

All the details get populated in the window. Select the License type from the drop down list
...
* [Invalidate HTTP Request Header Policy](/api-management/validate-http-header/)
* [Compress Response Policy](/api-management/compression-apply/)
* [Response Content Format Transformation Policy](/api-management/response-content-format-Copy/)
...
pick the credential make sure you have [configured the environments](/api-management/deployments-and-environments/) correctly and also 
