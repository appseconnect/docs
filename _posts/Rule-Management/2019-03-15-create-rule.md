...
2. Enter `Rule Name & Rule description` because they are marked as mandatory fields. The Status of the Rule
can be Released, Obsolete & Work in Progress. By default, the status of the rule is "released".
...
3. Click Continue button to go to the next page.
...
If the Rule Triggered for is selected as Scheduled, the scheduled property opens. 

Provide the Repeat Schedule (Minutely, Hourly, Daily) and the Repeat Interval (In Seconds) and the End Status. 
Click on the Continue Button.  

5. You will be redirected to a new page where you can Define criteria for a Rule.
...
6. In the Actions tab, you will provide actions to be taken regarding the rule.
...
7. `Confirm the Rule' is the last tab of rule creation process. Click `SAVE` and you will get to see the overview of the rule.
